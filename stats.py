...
BG, SCANLINE, PHOSPHOR, DIM, VERY_DIM = "#060d06", "#0a150a", "#b9ffb0", "#4d8f47", "#2a4d26"
ACCENT_CYAN, ACCENT_AMB, ACCENT_RED, BAR_FULL = "#7fffea", "#ffd166", "#ff6b6b", "#39d353"
...
def get_weather(city="Hyderabad"):
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as r:
            ...
            glyphs = {"thunder": "&#9889;", "rain": "&#9974;", "snow": "&#10052;", "cloud": "&#9729;", "sun": "&#9728;"}
...
def render(stats, weather=None):
    ...
    y = 48
    svg.append(f'<text x="{PAD}" y="{y-16}" fill="{VERY_DIM}" font-family="{MONO}" font-size="9">BROPAL-STATS-CARD v4.0 :: INITIALISED</text>')
    svg.append(f'<text x="{PAD}" y="{y+10}" fill="{ACCENT_CYAN}" font-family="{MONO}" font-size="30" font-weight="700">{stats["name"]}</text>')
    svg.append(f'<text x="{W-PAD}" y="{y+10}" fill="{DIM}" font-family="{MONO}" font-size="11" text-anchor="end">@{stats["username"]}</text>')
    
    y += 32
    svg.append(f'<text x="{PAD}" y="{y}" fill="{DIM}" font-family="{MONO}" font-size="11"><tspan fill="{ACCENT_AMB}">&#10095;</tspan>  {html.escape(stats["status"])}</text>')
    if weather:
        svg.append(f'<text x="{W-PAD}" y="{y}" fill="{ACCENT_AMB}" font-family="{MONO}" font-size="11" text-anchor="end">{weather["glyph"]} {weather["temp"]}°C  {weather["desc"]}</text>')
    
    y += 28
    svg.append(f'<text x="{PAD}" y="{y}" fill="{sig_col}" font-family="{MONO}" font-size="11">SIG {sig_str}</text>')
    svg.append(f'<text x="{W-PAD}" y="{y}" fill="{VERY_DIM}" font-family="{MONO}" font-size="10" text-anchor="end">UPTIME {stats["uptime"]}</text>')
    ...
    for lang, _, pct in stats["top_langs"]:
        svg.append(f'<text x="{PAD}" y="{y}" font-family="{MONO}" font-size="12"><tspan fill="{PHOSPHOR}">{lang:<14}</tspan> <tspan fill="{LANG_COLORS.get(lang, DIM)}">{_ascii_bar(pct)}</tspan> <tspan fill="{DIM}">{pct:>3}%</tspan></text>')
        y += 24
    ...
    for k, n, ts in [("PUSH", n, t) for n, t in stats["pushes"]]:
        ...
        svg.append(f'<text x="{PAD}" y="{y}" font-family="{MONO}" font-size="11.5"><tspan fill="{VERY_DIM}">[</tspan><tspan fill="{PHOSPHOR}">{k}</tspan><tspan fill="{VERY_DIM}">]</tspan><tspan fill="{PHOSPHOR}"> {html.escape(n[:36])}</tspan><tspan fill="{DIM}">  +{ago_s}</tspan></text>')
        y += 24
        
    y += 12
    s, y = draw_sec("LIFE STATS", y)
    svg.append(s)
    for k, v, c in [("REPOS", stats["repos"], ACCENT_CYAN), ("STARS", stats["stars"], ACCENT_AMB), ("FOLLOWERS", stats["followers"], PHOSPHOR)]:
        dots = "·" * (40 - len(k) - len(str(v)))
        svg.append(f'<text x="{PAD}" y="{y}" font-family="{MONO}" font-size="12"><tspan fill="{DIM}">{k} </tspan><tspan fill="{VERY_DIM}">{dots} </tspan><tspan fill="{c}" font-weight="600">{v}</tspan></text>')
        y += 24

    y += 20
    svg.append(f'<line x1="{PAD}" y1="{y}" x2="{W-PAD}" y2="{y}" stroke="{VERY_DIM}" stroke-width="0.8"/>')
    svg.append(f'<text x="{PAD}" y="{y+20}" fill="{VERY_DIM}" font-family="{MONO}" font-size="9">generated {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M")} UTC</text>')
...
def main():
    ...
    if stats:
        ...
        print(f'<svg width="400" height="50" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" fill="#060d06"/><text x="20" y="30" fill="red" font-family="monospace">ERR: Could not fetch stats</text></svg>')
...
def get_stats(username):
    ...
    return {
        "name": (user.get("name") or username)[:36], "username": user.get("login"),
        "repos": user.get("public_repos", 0), "followers": user.get("followers", 0), "stars": stars,
        "top_langs": [(l, c, round(c/tot*100)) for l, c in top5],
        "pushes": [(r["name"], r["pushed_at"]) for r in repos if not r.get("fork")][:2],
        "prs": [], "status": time_status(), "uptime": _account_uptime()
    }
